...
// NOTE: Authentication / API-key checks have been removed. This backend is
// now fully open - anyone can call any endpoint directly, no key required.
...
module.exports = async (req, res) => {
  ...
  if (path === '' || path === '/' || path === '/status') {
    return res.json({ status: 'online', name: 'Black Node ZW API', version: '1.0.0', base_url: 'https://blacknodezw.zone.id', endpoints: 40, timestamp: new Date().toISOString() });
  }
  ...
  if (shortMatch) {
    ...
    } catch (e) { return res.redirect('https://blacknodezw.zone.id'); }
  }
  ...
  try {
    ...
    if (path === '/instagram/download') {
      ...
      try {
        const snapResp = await axios.post('https://snapinsta.app/action_download.php',
          new URLSearchParams({ url: decodedUrl }),
          { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', Referer: 'https://snapinsta.app/', Origin: 'https://snapinsta.app' }, timeout: 20000 }
        );
        const html = typeof snapResp.data === 'string' ? snapResp.data : '';
        const videoMatch = html.match(/href="(https:\/\/[^"]*\.mp4[^"]*)"/);
        const imgMatch = html.match(/src="(https:\/\/[^"]*\.jpg[^"]*)"/);
        if (videoMatch) return ok(res, { type: 'video', download_url: videoMatch[1] });
        if (imgMatch) return ok(res, { type: 'image', download_url: imgMatch[1] });
      } catch (_) {}
      try {
        const r = await axios.post('https://v3.saveinsta.app/api/ajaxSearch',
          new URLSearchParams({ q: decodedUrl, t: 'media', lang: 'en' }),
          { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0' }, timeout: 20000 }
        );
        if (r.data?.data) {
          const d = r.data.data;
          if (d.medias?.length) return ok(res, { type: d.type || 'video', items: d.medias.map(m => ({ url: m.url, quality: m.quality || 'hd', extension: m.extension || 'mp4' })), download_url: d.medias[0]?.url });
        }
      } catch (_) {}
      return err(res, 'Could not download Instagram media. The post may be private or Instagram is blocking scrapers. Try again.');
    }
    ...
    // ── OTHER DOWNLOADERS ────────────────────────────────────────────────────
    if (path === '/facebook/download') {
      const { url, quality = 'hd' } = req.query;
      if (!url) return err(res, 'Missing: url', 400);
      const decodedUrl = decodeURIComponent(url);
      const cd = await cobaltDownload(decodedUrl);
      if (cd?.status === 'stream' || cd?.status === 'redirect') return ok(res, { download_url: cd.url, quality });
      if (cd?.status === 'picker' && cd.picker?.length) {
        const pick = quality === 'hd' ? cd.picker[0] : cd.picker[cd.picker.length - 1];
        return ok(res, { download_url: pick.url, quality, all_qualities: cd.picker.map(p => ({ url: p.url, quality: p.quality || 'unknown' })) });
      }
      try {
        const r = await axios.post('https://www.getfvid.com/downloader',
          new URLSearchParams({ url: decodedUrl }),
          { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', Referer: 'https://www.getfvid.com/', Origin: 'https://www.getfvid.com' }, timeout: 20000 }
        );
        const hdMatch = r.data.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"\s[^>]*>HD/i);
        const sdMatch = r.data.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"\s[^>]*>SD/i);
        const anyMatch = r.data.match(/href="(https:\/\/video[^\.]*\.facebook[^"]+)"/);
        const dlUrl = (quality === 'hd' ? hdMatch?.[1] : sdMatch?.[1]) || hdMatch?.[1] || sdMatch?.[1] || anyMatch?.[1];
        if (dlUrl) return ok(res, { download_url: dlUrl, quality: hdMatch ? 'hd' : 'sd' });
      } catch (_) {}
      try {
        const r = await axios.post('https://fdown.net/download.php',
          new URLSearchParams({ URLz: decodedUrl }),
          { headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', Referer: 'https://fdown.net/' }, timeout: 20000 }
        );
        const hdMatch = r.data.match(/id="hdlink"\s+href="([^"]+)"/);
        const sdMatch = r.data.match(/id="sdlink"\s+href="([^"]+)"/);
        if (hdMatch || sdMatch) return ok(res, { download_url_hd: hdMatch?.[1] || '', download_url_sd: sdMatch?.[1] || '', download_url: hdMatch?.[1] || sdMatch?.[1] });
      } catch (_) {}
      return err(res, 'Could not download Facebook video. Make sure the video is public.');
    }

    if (path === '/twitter/download') {
      const { url } = req.query;
      if (!url) return err(res, 'Missing: url', 400);
      const decodedUrl = decodeURIComponent(url);
      const cd = await cobaltDownload(decodedUrl);
      ...
      try {
        const r = await axios.get(`https://twitsave.com/info?url=${encodeURIComponent(decodedUrl)}`, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 15000 });
        const match = r.data.match(/downloadFile\("(https:\/\/[^"]+\.mp4[^"]*)"/);
        if (match) return ok(res, { download_url: match[1] });
      } catch (_) {}
      return err(res, 'Could not download. Make sure the tweet has a video and is public.');
    }
    ...
    if (path === '/url/shorten') {
      ...
      try {
        ...
        return ok(res, { original: url, short: `https://blacknodezw.zone.id/api/s/${alias}`, alias, clicks: snap.exists ? snap.data().clicks : 0 });
      } catch (e) { return err(res, e.message); }
    }
    ...
    if (path === '/paste/create') {
      ...
      try {
        ...
        return ok(res, { id, title, url: `https://blacknodezw.zone.id/api/paste/${id}`, expiry, created_at: new Date().toISOString() });
      } catch (e) { return err(res, e.message); }
    }
    ...
    if (path === '/pinterest/search') {
      const { q, count = '20' } = req.query;
      if (!q) return err(res, 'Missing: q', 400);
      try {
        const r = await axios.get('https://www.pinterest.com/search/pins/', {
          params: { q, rs: 'typed' },
          headers: {
        ...
        const matches = [...r.data.matchAll(/"images":\{"236x":\{"url":"([^"]+)"/g)];
        const hdMatches = [...r.data.matchAll(/"orig":\{"url":"([^"]+)"/g)];
        const titleMatches = [...r.data.matchAll(/"grid_description":"([^"]+)"/g)];
        const results = matches.slice(0, parseInt(count)).map((m, i) => ({
          thumbnail: m[1].replace(/\\/g, ''),
          image: (hdMatches[i]?.[1] || m[1]).replace(/\\/g, ''),
          title: titleMatches[i]?.[1] || q
        }));
        if (results.length) return ok(res, { query: q, total: results.length, images: results });
        const imgs = [];
        for (let i = 0; i < Math.min(parseInt(count), 10); i++) {
          const seed = encodeURIComponent(q) + i;
          imgs.push({ image: `https://source.unsplash.com/800x800/?${seed}`, thumbnail: `https://source.unsplash.com/400x400/?${seed}`, title: q });
        }
        return ok(res, { query: q, total: imgs.length, images: imgs, note: 'Pinterest blocked direct scraping. Showing Unsplash results instead.' });
      } catch (e) {
        return err(res, 'Pinterest search failed. Try again.');
      }
    }

    // ── DEEPSEEK AI ────────────────────────────────────────────────────
    if (path === '/ai/deepseek') {
      const { message, system = 'You are a helpful AI assistant.', model = 'deepseek-chat' } = req.body || {};
      if (!message) return err(res, 'Missing: message', 400);
      const dsKey = process.env.DEEPSEEK_API_KEY;
      if (!dsKey) return err(res, 'Set DEEPSEEK_API_KEY in Vercel environment variables. Get free key at platform.deepseek.com', 400);
      try {
        const r = await axios.post('https://api.deepseek.com/chat/completions',
          { model, messages: [{ role: 'system', content: system }, { role: 'user', content: message }], max_tokens: 2048 },
          { headers: { 'Authorization': `Bearer ${dsKey}`, 'Content-Type': 'application/json' }, timeout: 30000 }
        );
        return ok(res, { message, response: r.data.choices[0]?.message?.content || '', model: r.data.model, tokens_used: r.data.usage?.total_tokens || 0 });
      } catch (e) { return err(res, e.response?.data?.error?.message || e.message); }
    }

    // ── CLAUDE AI ──────────────────────────────────────────────────────
    if (path === '/ai/claude') {
      const { message, system = 'You are a helpful AI assistant.', model = 'claude-3-haiku-20240307' } = req.body || {};
      if (!message) return err(res, 'Missing: message', 400);
      const claudeKey = process.env.ANTHROPIC_API_KEY;
      if (!claudeKey) return err(res, 'Set ANTHROPIC_API_KEY in Vercel environment variables. Get key at console.anthropic.com', 400);
      try {
        const r = await axios.post('https://api.anthropic.com/v1/messages',
          { model, max_tokens: 1024, system, messages: [{ role: 'user', content: message }] },
          { headers: { 'x-api-key': claudeKey, 'anthropic-version': '2023-06-01', 'Content-Type': 'application/json' }, timeout: 30000 }
        );
        return ok(res, { message, response: r.data.content[0]?.text || '', model: r.data.model, tokens_used: (r.data.usage?.input_tokens || 0) + (r.data.usage?.output_tokens || 0) });
      } catch (e) { return err(res, e.response?.data?.error?.message || e.message); }
    }

    // ── GROK AI ────────────────────────────────────────────────────────
    if (path === '/ai/grok') {
      const { message, system = 'You are Grok, a helpful AI assistant.', model = 'grok-beta' } = req.body || {};
      if (!message) return err(res, 'Missing: message', 400);
      const grokKey = process.env.GROK_API_KEY || process.env.XAI_API_KEY;
      if (!grokKey) return err(res, 'Set GROK_API_KEY in Vercel environment variables. Get key at console.x.ai', 400);
      try {
        const r = await axios.post('https://api.x.ai/v1/chat/completions',
          { model, messages: [{ role: 'system', content: system }, { role: 'user', content: message }], max_tokens: 1024 },
          { headers: { 'Authorization': `Bearer ${grokKey}`, 'Content-Type': 'application/json' }, timeout: 30000 }
        );
        return ok(res, { message, response: r.data.choices[0]?.message?.content || '', model: r.data.model, tokens_used: r.data.usage?.total_tokens || 0 });
      } catch (e) { return err(res, e.response?.data?.error?.message || e.message); }
    }
    ...
    if (path === '/ai/gemini') {
      ...
      if (!geminiKey) return err(res, 'Set GEMINI_API_KEY in Vercel environment variables. Get free key at aistudio.google.com', 400);
      try {
        const r = await axios.post(
          `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${geminiKey}`,
          { contents: [{ parts: [{ text: message }] }], generationConfig: { maxOutputTokens: 1024 } },
          { headers: { 'Content-Type': 'application/json' }, timeout: 30000 }
        );
        const response = r.data.candidates?.[0]?.content?.parts?.[0]?.text || '';
        return ok(res, { message, response, model, tokens_used: r.data.usageMetadata?.totalTokenCount || 0 });
      } catch (e) { return err(res, e.response?.data?.error?.message || e.message); }
    }

    // ── CHATGPT / OPENAI GPT-4 ───────────────────────────────────────────
    if (path === '/ai/chatgpt') {
      const { message, system = 'You are ChatGPT, a helpful AI assistant.', model = 'gpt-4o-mini' } = req.body || {};
      if (!message) return err(res, 'Missing: message', 400);
      const openaiKey = process.env.OPENAI_API_KEY;
      if (!openaiKey) return err(res, 'Set OPENAI_API_KEY in Vercel environment variables. Get key at platform.openai.com', 400);
      try {
    ...
    if (path === '/ai/video') {
      const prompt = req.body?.prompt || req.query?.prompt;
      const duration = parseInt(req.body?.duration || req.query?.duration || '5');
      if (!prompt) return err(res, 'Missing: prompt', 400);
      const stabilityKey = process.env.STABILITY_API_KEY;
      if (stabilityKey) {
        try {
          const r = await axios.post('https://api.stability.ai/v2beta/image-to-video',
            { image: `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1024&height=576&nologo=true`, seed: 0, cfg_scale: 1.8, motion_bucket_id: 127 },
            { headers: { 'Authorization': `Bearer ${stabilityKey}`, 'Content-Type': 'application/json' }, timeout: 30000 }
          );
          if (r.data.id) {
            return ok(res, { prompt, generation_id: r.data.id, status: 'processing', check_url: `https://blacknodezw.zone.id/api/ai/video/result?id=${r.data.id}`, note: 'Video is being generated. Check check_url in 30-60 seconds.' });
          }
        } catch (_) {}
      }
      ...
      if (lumaKey) {
        try {
          const r = await axios.post('https://api.lumalabs.ai/dream-machine/v1/generations',
            { prompt, aspect_ratio: '16:9', loop: false },
            { headers: { 'Authorization': `Bearer ${lumaKey}`, 'Content-Type': 'application/json' }, timeout: 30000 }
          );
          return ok(res, { prompt, generation_id: r.data.id, status: r.data.state, model: 'luma-dream-machine', note: 'Video generation started. Takes 1-3 minutes.' });
        } catch (_) {}
      }
      return ok(res, {
        prompt, status: 'no_key',
        message: 'AI video generation requires an API key.',
        options: [
          { provider: 'Stability AI', key_name: 'STABILITY_API_KEY', signup: 'platform.stability.ai', free_credits: true },
          { provider: 'Luma AI', key_name: 'LUMAAI_API_KEY', signup: 'lumalabs.ai/dream-machine', free_credits: true },
          { provider: 'RunwayML', key_name: 'RUNWAY_API_KEY', signup: 'runwayml.com', free_credits: true }
        ],
        note: 'Set any of these keys in Vercel environment variables.'
      });
    }

    // ── AI VIDEO RESULT CHECK ────────────────────────────────────────────────
    if (path === '/ai/video/result') {
      const { id } = req.query;
      if (!id) return err(res, 'Missing: id', 400);
      const stabilityKey = process.env.STABILITY_API_KEY;
      if (stabilityKey) {
        try {
          const r = await axios.get(`https://api.stability.ai/v2beta/image-to-video/result/${id}`,
            { headers: { 'Authorization': `Bearer ${stabilityKey}`, Accept: 'video/*' }, responseType: 'arraybuffer', timeout: 30000 }
          );
          if (r.status === 200) {
            const b64 = Buffer.from(r.data).toString('base64');
            return ok(res, { id, status: 'complete', video_base64: `data:video/mp4;base64,${b64}` });
          }
          return ok(res, { id, status: 'processing', note: 'Still generating. Try again in 10 seconds.' });
        } catch (_) {}
      }
      return err(res, 'Set STABILITY_API_KEY to check video generation status.');
    }

    // ── AI VISION / IMAGE ANALYSIS ────────────────────────────────────
    if (path === '/ai/vision') {
      const { image_url, prompt = 'Describe this image in detail.' } = req.body || {};
      if (!image_url) return err(res, 'Missing: image_url', 400);
      const openaiKey = process.env.OPENAI_API_KEY;
      if (openaiKey) {
        try {
          const r = await axios.post('https://api.openai.com/v1/chat/completions',
            { model: 'gpt-4o-mini', messages: [{ role: 'user', content: [{ type: 'text', text: prompt }, { type: 'image_url', image_url: { url: image_url } }] }], max_tokens: 1024 },
            { headers: { 'Authorization': `Bearer ${openaiKey}`, 'Content-Type': 'application/json' }, timeout: 30000 }
          );
          return ok(res, { image_url, prompt, response: r.data.choices[0]?.message?.content || '', model: 'gpt-4o-mini' });
        } catch (e) { return err(res, e.response?.data?.error?.message || e.message); }
      }
      ...
      if (geminiKey) {
        try {
          const r = await axios.post(
            ...
            { contents: [{ parts: [{ text: prompt }, { inline_data: { mime_type: 'image/jpeg', data: image_url } }] }] },
            { headers: { 'Content-Type': 'application/json' }, timeout: 30000 }
          );
          return ok(res, { image_url, prompt, response: r.data.candidates?.[0]?.content?.parts?.[0]?.text || '', model: 'gemini-1.5-flash' });
        } catch (_) {}
      }
      return err(res, 'Set OPENAI_API_KEY or GEMINI_API_KEY for image analysis.');
    }

    // ── AI SUMMARIZER ──────────────────────────────────────────────────
    if (path === '/ai/summarize') {
      const { text, url: pageUrl, length = 'medium' } = req.body || {};
      if (!text && !pageUrl) return err(res, 'Missing: text or url', 400);
      let content = text;
      if (pageUrl && !text) {
        try {
          const r = await axios.get(pageUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 12000 });
          const cheerio = require('cheerio');
          const $ = cheerio.load(r.data);
          $('script,style,nav,footer,header,aside').remove();
          content = $('body').text().replace(/\s+/g, ' ').trim().slice(0, 8000);
        } catch (e) { return err(res, 'Could not fetch URL content: ' + e.message); }
      }
      const maxWords = { short: 100, medium: 250, long: 500 }[length] || 250;
      const prompt = `Summarize the following text in about ${maxWords} words:\n\n${content}`;
      const openaiKey = process.env.OPENAI_API_KEY;
      const groqKey = process.env.GROQ_API_KEY;
      try {
        if (openaiKey) {
          const r = await axios.post('https://api.openai.com/v1/chat/completions', { model: 'gpt-3.5-turbo', messages: [{ role: 'user', content: prompt }], max_tokens: 600 }, { headers: { 'Authorization': `Bearer ${openaiKey}`, 'Content-Type': 'application/json' }, timeout: 25000 });
          return ok(res, { summary: r.data.choices[0]?.message?.content || '', length, model: 'gpt-3.5-turbo' });
        }
        if (groqKey) {
          const r = await axios.post('https://api.groq.com/openai/v1/chat/completions', { model: 'llama3-8b-8192', messages: [{ role: 'user', content: prompt }], max_tokens: 600 }, { headers: { 'Authorization': `Bearer ${groqKey}`, 'Content-Type': 'application/json' }, timeout: 20000 });
          return ok(res, { summary: r.data.choices[0]?.message?.content || '', length, model: 'llama3-8b' });
        }
        const r = await axios.get(`https://text.pollinations.ai/${encodeURIComponent(prompt)}`, { timeout: 25000 });
        return ok(res, { summary: typeof r.data === 'string' ? r.data : JSON.stringify(r.data), length, model: 'pollinations' });
      } catch (e) { return err(res, e.message); }
    }
    ...
    if (path === '/country') {
      ...
      if (!name && !code) return err(res, 'Missing: name or code', 400);
      try {
        const query = code ? `alpha/${code}` : `name/${encodeURIComponent(name)}`;
        const r = await axios.get(`https://restcountries.com/v3.1/${query}`, { timeout: 10000 });
        const c = Array.isArray(r.data) ? r.data[0] : r.data;
        return ok(res, { name: c.name?.common, official_name: c.name?.official, capital: c.capital?.[0] || '', region: c.region, subregion: c.subregion, population: c.population, area_km2: c.area, languages: Object.values(c.languages || {}), currencies: Object.values(c.currencies || {}).map(cu => ({ name: cu.name, symbol: cu.symbol })), timezones: c.timezones, calling_codes: c.idd?.suffixes?.map(s => (c.idd?.root || '') + s) || [], flag_emoji: c.flag, flag_url: c.flags?.png || '', continent: c.continents?.[0] || '' });
      } catch (e) { return err(res, `Country not found: ${name || code}`); }
    }

    // ── NOT FOUND ─────────────────────────────────────────────────────────────
    return res.status(404).json({ status: 404, error: `Endpoint not found: ${path}`, docs: 'https://blacknodezw.zone.id', available_endpoints: ['/tiktok/download','/tiktok/user','/tiktok/search','/tiktok/trending','/youtube/mp3','/youtube/mp4','/youtube/info','/youtube/channel','/instagram/download','/instagram/user','/facebook/download','/twitter/download','/pinterest/download','/spotify/track','/image/search','/image/generate','/image/removebg','/ai/chat','/ai/generate','/ai/tts','/ai/translate','/ai/music','/weather','/currency/convert','/qrcode/generate','/ip/lookup','/url/shorten','/news','/dictionary','/github/user','/screenshot','/fun','/movie/search','/movie/details','/movie/trending','/whatsapp/send','/telegram/send','/telegram/file','/storage/upload','/phone/lookup','/email/validate','/paste/create','/random/password','/random/uuid','/random/color'] });
