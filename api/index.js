...
module.exports = async function handler(req, res) {
  ...
  const parsed = new URL(req.url || '/', 'http://localhost'); const route = parsed.pathname.replace(/^\/api(?=\/|$)/, '').replace(/\/$/, '') || '/'; const q = parsed.searchParams;
  try {
    ...
    if (req.method === 'GET' && route === '/qrcode/generate') { const text = q.get('text') || q.get('data'); if (!text) return missing(res, 'text'); const size = Math.min(Math.max(Number(q.get('size') || 300), 100), 1000); const format = (q.get('format') || 'json').toLowerCase(); if (format === 'svg') { const svg = await QRCode.toString(text, { type: 'svg', margin: 2, width: size }); res.setHeader('Content-Type', 'image/svg+xml; charset=utf-8'); return res.end(svg); } if (format === 'png') { const png = await QRCode.toDataURL(text, { margin: 2, width: size }); return ok(res, { text, format, qr_code: png }); } const svg = await QRCode.toString(text, { type: 'svg', margin: 2, width: size }); return ok(res, { text, format: 'json', qr_code: svg }); }
    ...
    if (req.method === 'GET' && route === '/random/uuid') { const count = Math.min(Math.max(Number(q.get('count') || 1), 1), 100); const uuids = Array.from({ length: count }, () => id()); return ok(res, { uuid: uuids[0], uuids, count }); }
    ...
    if (req.method === 'POST' && route === '/paste/create') { const body = await readBody(req); const content = typeof body.content === 'string' ? body.content : ''; if (!content) return missing(res, 'content'); const paste = { id: id().slice(0, 8), content, title: String(body.title || '').slice(0, 120), created_at: now(), expires_at: body.expires_at || body.expiry || null }; pastes.set(paste.id, paste); return ok(res, { id: paste.id, url: `/api/paste/${paste.id}`, created_at: paste.created_at, expires_at: paste.expires_at }, 201); }
    ...
    if (req.method === 'POST' && route === '/url/shorten') { const body = await readBody(req); const target = body.url || q.get('url'); try { new URL(target); } catch { return fail(res, 400, 'A valid url is required'); } const alias = String(body.alias || '').trim() || id().slice(0, 8); if (!/^[A-Za-z0-9_-]{3,40}$/.test(alias)) return fail(res, 400, 'alias must contain 3-40 letters, numbers, underscores, or hyphens'); if (shortUrls.has(alias)) return fail(res, 409, 'Alias already exists'); shortUrls.set(alias, { url: target, created_at: now(), clicks: 0 }); return ok(res, { alias, url: `/api/s/${alias}`, target }, 201); }
