...
// PATCH - add to index.js before the NOT FOUND section

    // ── INSTAGRAM DOWNLOAD FIX ───────────────────────────────────────────────
    // (already in file, replacing with better version)

    // ── FACEBOOK DOWNLOAD FIX ────────────────────────────────────────────────
    // Uses getfvid.com scrape + cobalt fallback

    // ── PINTEREST IMAGE SEARCH (NEW) ─────────────────────────────────────────
    ...
    // ── CHATGPT / OPENAI GPT-4 (NEW explicit endpoint) ───────────────────────
    if (path === '/ai/chatgpt') {
    ...
    if (path === '/ai/video') {
      ...
      if (!prompt) return err(res, 'Missing: prompt', 400);
      // Method 1: Stability AI video (needs key)
      ...
      if (stabilityKey) {
        try {
          ...
          if (r.data.id) {
            return ok(res, { prompt, generation_id: r.data.id, status: 'processing', check_url: `https://api-blacknode-zw.vercel.app/api/ai/video/result?id=${r.data.id}`, note: 'Video is being generated. Check check_url in 30-60 seconds.' });
          }
        } catch (_) {}
      }
      // Method 2: Luma AI Dream Machine (needs key)
