...
module.exports = async (req, res) => {
  ...
  if (path === '' || path === '/' || path === '/status') {
    return res.json({ status: 'online', name: 'Black Node ZW API', version: '1.0.0', base_url: 'https://api-blacknode-zw.vercel.app', endpoints: 30, timestamp: new Date().toISOString() });
  }
  ...
  try {
    ...
    return res.status(404).json({ status: 404, error: `Endpoint not found: ${path}`, docs: 'https://api-blacknode-zw.vercel.app' });
